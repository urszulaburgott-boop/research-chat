...
function makeBaseUrl() {
  ...
  // fallback pro server-side buildy (Vercel)
...
export async function listLinks(chatId) {
  const { data, error } = await supabase
    .from("chat_links")
    .select("id, chat_id, role, internal_name, url, nickname, multi, token, deleted_at")
    .eq("chat_id", chatId)
    .is("deleted_at", null)
    .order("created_at", { ascending: true });
  if (error) throw error;
...
/** ========= ZPRÁVY ========= */
// jednotná verze, kterou používáme v chatu (veřejná i DM)
export async function sendMessage(chatId, text, linkId = null, recipientLinkId = null) {
  const row = {
    chat_id: chatId,
    text,
    // author_link_id: linkId || null, // až budeš chtít ukládat ID autora
    recipient_link_id: recipientLinkId || null, // null = veřejná, jinak DM
  };
  const { data, error } = await supabase
    .from("chat_messages")
    .insert([row])
